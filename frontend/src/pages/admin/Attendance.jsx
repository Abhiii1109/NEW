...
const AdminAttendance = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <div className="relative group w-full sm:w-auto">
              ...
              <input type="date" className="w-full pl-10 pr-4 py-2 bg-white rounded-lg text-xs font-bold text-foreground outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all shadow-subtle" />
            </div>
            <select className="w-full sm:w-auto px-4 py-2 bg-white rounded-lg text-xs font-bold text-foreground outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all shadow-subtle min-w-[180px]">
      ...
      <Card title="Attendance Records" subtitle="Daily personnel logs" className="overflow-x-auto shadow-subtle">
                ...
                <tr key={record.id} className={cn("hover:bg-slate-50/50 transition-colors", i % 2 !== 0 && "bg-slate-50/20")}>
                  <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                          <img src={record.avatar || `https://ui-avatars.com/api/?name=${record.name}&background=0f172a&color=fff`} alt="" className="w-8 h-8 rounded-lg bg-slate-100" />
